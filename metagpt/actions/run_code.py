...
## File To Rewrite:
Determine the ONE file to rewrite in order to fix the error, for example, xyz.cpp, or test_xyz.cpp
## Status:
...
class RunCode(Action):
    ...
    @classmethod
    async def run_cpp_code(cls, working_directory, command=[]) -> Tuple[str, str]:
        # Start the C++ compilation process
        compile_command = ["g++", "-o", "program", *command]
        process_compile = subprocess.Popen(
            compile_command, cwd=working_directory, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        _, compile_err = process_compile.communicate()
        if compile_err:
            return "", compile_err.decode("utf-8")

        # Run the compiled C++ program
        run_command = ["./program"]
        process_run = subprocess.Popen(
            run_command, cwd=working_directory, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        run_out, run_err = process_run.communicate()
        return run_out.decode("utf-8"), run_err.decode("utf-8")
    ...
    async def run(self, *args, **kwargs) -> RunCodeResult:
        ...
        elif self.context.script_language == "cpp":
            outs, errs = await self.run_cpp_code(
                command=self.context.command,
                working_directory=self.context.working_directory
            )
